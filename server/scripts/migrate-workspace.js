...
async function add(table, name, definition) {
  ...
  if (!cols.some(c => c.Field === name)) await db.query(`ALTER TABLE \`${table}\` ADD \`${name}\` ${definition}`);
}
async function migrate() {
  ...
  function root(id) { const seen=new Set(); while(teams.find(t=>t.teams_id===id)?.teams_teams_id) { if(seen.has(id)) throw Error('Circular team hierarchy'); seen.add(id); id=teams.find(t=>t.teams_id===id).teams_teams_id; } return id; }
  for(const t of teams) await db.query('UPDATE teams SET workspace_id=? WHERE teams_id=? AND workspace_id IS NULL',[root(t.teams_id),t.teams_id]);
  const users=await db.query('SELECT user_id,teams_teams_id,role,workspace_id FROM user ORDER BY user_id');
  for(const u of users) {
    ...
    await db.query('UPDATE user SET workspace_id=? WHERE user_id=? AND workspace_id IS NULL',[wid,u.user_id]);
    await db.query('INSERT IGNORE INTO workspace_members (workspace_id,user_id,role) VALUES (?,?,?)',[wid,u.user_id,u.user_id===owner.user_id?'OWNER':['ADMIN','TL'].includes(u.role)?'MANAGER':'AGENT']);
    if(u.workspace_id===null)await db.query('INSERT IGNORE INTO team_members (team_id,user_id) VALUES (?,?)',[u.teams_teams_id,u.user_id]);
  }
  await db.query('UPDATE client c JOIN user u ON u.user_id=c.user_user_id SET c.workspace_id=u.workspace_id,c.created_by=c.user_user_id WHERE c.workspace_id IS NULL');
  ...
  if(indexes.some(i=>i.Key_name==='uq_client_contact_type')) await db.query('ALTER TABLE contact_point DROP INDEX uq_client_contact_type');
  if(!indexes.some(i=>i.Key_name==='uq_contact_value')) await db.query('ALTER TABLE contact_point ADD UNIQUE KEY uq_contact_value (client_id,contact_type,contact_info)');
  await db.query(`INSERT IGNORE INTO contact_point (client_id,contact_type,contact_info) SELECT client_id,'Email',email FROM client_crm_details WHERE email<>''`);
  await db.query(`INSERT IGNORE INTO contact_point (client_id,contact_type,contact_info) SELECT client_id,'Phone',phone FROM client_crm_details WHERE phone<>''`);
  ...
  for(const name of ['access_token','page_id','conversation_id_ext','client_psid']) await db.query(`ALTER TABLE conversation MODIFY ${name} VARCHAR(255) NULL`);
  for(const name of ['token_refreshed_at','page_created_at','last_message_at','last_message_who']) await db.query(`ALTER TABLE conversation MODIFY ${name} DATETIME NULL`);
  const convIndex=await db.query('SHOW INDEX FROM conversation');
  if(!convIndex.some(i=>i.Key_name==='uq_channel_recipient')) await db.query('ALTER TABLE conversation ADD UNIQUE KEY uq_channel_recipient (connection_id,client_psid)');
  ...
  if(!msgIndex.some(i=>i.Key_name==='uq_provider_message')) await db.query('ALTER TABLE message ADD UNIQUE KEY uq_provider_message (conversation_conversation_id,provider_message_id)');
  if(!msgIndex.some(i=>i.Key_name==='uq_send_request')) await db.query('ALTER TABLE message ADD UNIQUE KEY uq_send_request (conversation_conversation_id,request_id)');
  console.log('Shared workspace, contact points, activities and Inbox schema ready. Existing records preserved.');
}
migrate().catch(e=>{console.error(e.code||e.message);process.exitCode=1;}).finally(()=>db.close());
