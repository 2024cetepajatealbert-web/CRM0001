...
async function run() {
  ...
  for (const [code, expected] of [[190, /new Page access token/], [200, /permissions/], [100, /personal or app token/], [4, /limiting/], [999, /Page token/]]) {
    global.fetch = async () => ({ ok: false, status: 400, json: async () => ({ error: { code, error_subcode: 463, message: secret } }) });
  ...
  await assert.rejects(messenger.graph('123/messages', secret, { message: { text: 'test' } }), error => {
    assert.match(publicResponse(error).message, /before sending again/);
    assert.equal(error.providerRejected, undefined);
    return true;
  });
  global.fetch = async () => ({ ok: true, json: async () => { throw new Error(secret); } });
  await assert.rejects(messenger.graph('me', secret), error => /could not validate/.test(publicResponse(error).message));
  global.fetch = async () => ({ ok: true, json: async () => null });
  await assert.rejects(messenger.graph('me', secret), error => /unexpected response/.test(publicResponse(error).message));
  ...
  assert.throws(() => messenger.encrypt({ pageToken: secret }), error => {
    const response = publicResponse(error);
  ...
  await assert.rejects(messenger.graph('me', secret), error => /META_GRAPH_VERSION/.test(publicResponse(error).message));
  assert.equal(publicResponse(new Error(secret)).message, 'The server could not complete that request.');
  console.log('PASS: safe Meta errors, network failures, configuration errors, success, and secret redaction.');
}

run().catch(error => { console.error(error); process.exitCode = 1; }).finally(() => {
  global.fetch = originalFetch;
  if (originalVersion === undefined) delete process.env.META_GRAPH_VERSION; else process.env.META_GRAPH_VERSION = originalVersion;
  if (originalKey === undefined) delete process.env.CONNECTION_ENCRYPTION_KEY; else process.env.CONNECTION_ENCRYPTION_KEY = originalKey;
});
