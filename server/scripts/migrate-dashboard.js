...
async function migrate() {
  ...
  if (!columns.some(c => c.Field === 'title')) await db.query("ALTER TABLE tasks ADD title VARCHAR(160) NOT NULL DEFAULT ''");
  if (!columns.some(c => c.Field === 'notes')) await db.query('ALTER TABLE tasks ADD notes TEXT NULL');
  console.log('Dashboard schema ready. Existing records preserved.');
}
migrate().catch(error => { console.error('Migration failed:', error.code || error.message); process.exitCode = 1; }).finally(() => db.close());
