...
async function run() {
  for (const [url, expected] of [
    ...
    await context.authForTest.login({
      email: "test@example.invalid",
      password: "test-only-password",
    });
    await context.authForTest.signup({ email: "test@example.invalid" });
    await context.window.cramApi.request("/api/crm/workspace", {
    ...
    context.fetch = async () => {
      throw new Error("Technical network error");
    };
  ...
  const fileContext = vm.createContext({
    location: new URL("file:///C:/CRAM/index.html"),
    window: {},
    fetch: () => assert.fail("File mode must not send credentials"),
  });
  vm.runInContext(source, fileContext);
  await assert.rejects(
    fileContext.window.cramApi.request("/api/auth/login"),
    /not directly as an HTML file/,
  );
  for (const mode of ["development", "production"]) {
    const options = corsOptions({
      NODE_ENV: mode,
      CLIENT_ORIGIN: "https://cram.example",
      PORT: "4000",
    });
  ...
  console.log(
    "PASS: Live Server and same-origin API routing, login/signup, safe errors, and restricted development CORS.",
  );
}
