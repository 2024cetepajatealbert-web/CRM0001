...
(() => {
  ...
  function enhance() {
    document.querySelectorAll('input[type=password]:not([data-password-control])').forEach(input => {
      ...
      button.setAttribute('aria-controls', input.id); setVisible(input, button, false); wrapper.append(button);
      button.addEventListener('click', event => { event.preventDefault(); setVisible(input, button, input.type === 'password'); });
    });
    document.querySelectorAll('.dialog-close,.close-auth,.close-onboarding').forEach(button => {
      if (button.dataset.centeredClose) return;
      button.dataset.centeredClose = 'true'; button.type = 'button';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', event => { if (event.target.closest('.close-auth,.close-onboarding,.open-login,.open-signup,.switch-login')) hidePasswords(); });
  });
