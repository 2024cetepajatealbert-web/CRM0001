...
(() => {
  const icons = {
    ...
    clients: '<circle cx="9" cy="8" r="3"/><path d="M3 21v-3a6 6 0 0 1 12 0v3M17 5a3 3 0 0 1 0 6m1 4a5 5 0 0 1 3 5"/>',
    ...
    calendar: '<rect x="3" y="5" width="18" height="16" rx="2"/><path d="M7 3v4m10-4v4M3 11h18m-13 5h3"/>',
  ...
  const money = value => value == null || value === '' ? 'Budget not set' : new Intl.NumberFormat('en-PH', { style:'currency', currency:'PHP', maximumFractionDigits:0 }).format(Number(value));
  const date = value => new Date(value).toLocaleString('en-PH', {month:'short', day:'numeric', hour:'numeric', minute:'2-digit'});
  ...
  const nav = [['Overview','overview'],['Clients','clients'],['Inbox','clients'],['Activities','calendar'],['Teams','clients'],['Analytics','chart'],['Connect','home']];
  ...
  const toast = message => { const el=document.querySelector('#toast'); el.textContent=message; el.classList.add('show'); clearTimeout(toastTimer); toastTimer=setTimeout(()=>el.classList.remove('show'),4000); };
  async function api(path, method='GET', body) {
    const {response,result}=await window.cramApi.request(`/api/crm${path}`, {method, headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`}, ...(body ? {body:JSON.stringify(body)} : {})});
    if(response.status===401) { logout(); throw new Error(result.message); }
    if(!response.ok) throw new Error(result.message || 'Could not save your changes. Please try again.');
    return result;
  }
  function logout() {
    window.cramCollab?.reset(); window.cramRecords?.reset();
    generation++; token=''; data=null; sessionStorage.removeItem('cram.token');
  ...
  async function open() {
    ...
    try { await load(); } catch(e) {
      if(token) root().innerHTML=`<div class="workspace-loading"><h2>Unable to load your workspace</h2><p>${esc(e.message)}</p><button class="btn-primary" data-action="refresh">Try again</button> <button class="quiet-button" data-action="logout">Log out</button></div>`;
  ...
  function stats() {
    ...
    const conversations=data.collaboration.conversations.filter(c=>c.status==='Open').length;
    const appointments=data.tasks.filter(t=>pending(t)&&isAppointment(t)&&new Date(t.dueAt)>=new Date()).length;
    return `<section class="metric-grid" aria-label="Workspace totals">${[
      ...
      ['clock','Due today',due,overdue?`${overdue} overdue task${overdue===1?'':'s'}`:'Your next steps, in one place','Follow-ups'],
      ['calendar','Upcoming appointments',appointments,'Site visits & online meetings','Appointments'],
      ['clients','Open conversations',conversations,'Keep every conversation moving','Inbox'],
    ].map(([i,label,count,caption,target])=>`<button class="metric-card" data-page="${target}"><span class="metric-icon">${icon(i)}</span><span class="metric-label">${label}</span><strong>${count}</strong><small>${caption}</small><span class="metric-arrow">↗</span></button>`).join('')}</section>`;
  }
  function taskList(tasks) {
    if(!tasks.length) return empty('A little room to plan ahead','Schedule the next conversation, call, or site visit.','add-task','Schedule a task');
    return `<div class="activity-list">${tasks.map(t=>{
  ...
  function clientTable(clients, recent=false) {
    if(!clients.length) return empty(query||filter?'No matching clients':'Your next relationship starts here',query||filter?'Try another name, email, or phone number.':'Add an inquiry from social media, a referral, an event, or a walk-in.', 'add-client','Add a client');
    return `<div class="table-wrap"><table class="workspace-table"><thead><tr><th>Client</th><th>Phone</th><th>Assigned to</th><th>Date added</th><th>Next step</th><th aria-label="Actions"></th></tr></thead><tbody>${clients.map(c=>{
  ...
  function contactSummary() {
    const counts=[['With email',data.clients.filter(c=>c.email).length],['With phone',data.clients.filter(c=>c.phone).length],['With conversations',new Set(data.collaboration.conversations.map(c=>c.clientId)).size]];
    return '<div class="source-chart">'+counts.map(([label,count])=>`<div><span>${label}</span><strong>${count}</strong><div class="bar-track"><i style="width:${data.clients.length?count/data.clients.length*100:0}%"></i></div></div>`).join('')+'</div>';
  }
  function overview() {
    const due=data.tasks.filter(t=>pending(t)&&new Date(t.dueAt)<=new Date(new Date().setHours(23,59,59,999)));
    const upcoming=data.tasks.find(t=>pending(t)&&isAppointment(t)&&new Date(t.dueAt)>=new Date());
  ...
  function pageContent() {
    const matches=(...values)=>values.some(v=>String(v||'').toLowerCase().includes(query.toLowerCase()));
    const helpers={api,load,render,navigate,showForm,esc,icon,date,money,empty,taskList,clientTable,stats,toast,root,dialog,query,filter,logout};
    ...
    if(page==='Follow-ups'||page==='Appointments') {
      let tasks=data.tasks.filter(t=>(page!=='Appointments'||isAppointment(t))&&matches(t.title,t.clientName,t.type)&&(!filter||(filter==='Overdue'?pending(t)&&new Date(t.dueAt)<new Date():t.status===filter)));
  ...
  function render() {
    ...
    const role={OWNER:'Workspace owner',MANAGER:'Team manager',AGENT:'Sales agent'}[data.user.workspaceRole]||'Team member';
  ...
  function navigate(target) { if(['Follow-ups','Appointments'].includes(target)) {window.cramCollab?.setActivityView(target==='Appointments'?'Appointments':'Upcoming');target='Activities';} if(!nav.some(([name])=>name===target)) return; page=target; query=''; filter=''; render(); window.scrollTo({top:0,behavior:'smooth'}); }
  const options = (values, selected='') => values.map(v=>`<option value="${esc(v)}" ${v===selected?'selected':''}>${esc(v)}</option>`).join('');
  function clientForm(client={}) {
    const field=(label,name,max,required=false,type='text',extra='')=>`<label class="${extra}"><span class="field-label">${label}${required?' <span aria-hidden="true">*</span>':''}</span><input name="${name}" type="${type}" maxlength="${max}" value="${esc(client[name]||'')}" ${required?'required':''}></label>`;
  ...
  function showForm(kind, clientId=null, appointment=false, conversationId=null) {
    ...
    if(kind==='client'&&client) window.cramCollab?.contactEditor(dialog(),client,data,{api,load,toast,esc});
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.addEventListener('click', async event=>{
      ...
      if(action==='open-chat') {window.cramCollab.selectClient(Number(button.dataset.id));navigate('Inbox');return;}
      if(action==='edit-client') return showForm('client',Number(button.dataset.id));
      if(action==='add-task'||action==='add-appointment') return showForm('task',Number(button.dataset.id)||null,action==='add-appointment',Number(button.dataset.conversation)||null);
      button.disabled=true;
      try {
        if(action==='complete') { const task=data.tasks.find(t=>t.id===Number(button.dataset.id)); await api(`/tasks/${task.id}`,'PATCH',{status:pending(task)?'Completed':'Pending'}); }
        await load();
    ...
    function filterRecords(event) {
      ...
      if(input.id==='workspace-search') query=input.value; else filter=input.value;
      const active=input.id;
      const caret=input.selectionStart;
      document.querySelector('#workspace-content').innerHTML=pageContent();
      const replacement=document.getElementById(active); replacement.focus();
      if(caret!==null&&replacement.setSelectionRange) replacement.setSelectionRange(caret,caret);
    }
    root().addEventListener('input',event=>{if(event.target.id==='workspace-search') filterRecords(event);});
    root().addEventListener('change',event=>{if(event.target.id==='workspace-filter') filterRecords(event);});
    dialog().addEventListener('change',event=>{if(event.target.name==='clientId')dialog().querySelector('[name=conversationId]')?.remove();});
    dialog().addEventListener('submit',async event=>{
      ...
      if(formKind==='client'&&!editingId&&!values.email.trim()&&!values.phone.trim()) { error.textContent='Add an email address or phone number.'; return; }
      if(formKind==='task') { const due=new Date(values.dueAt); if(!Number.isFinite(due.getTime())) {error.textContent='Choose a valid due date.';return;} values.dueAt=due.toISOString(); }
      button.disabled=true;
      try {
        await api(formKind==='client'?`/clients${editingId?'/'+editingId:''}`:'/tasks',editingId?'PUT':'POST',values);
        dialog().close(); toast(formKind==='client'?'Client saved.':'Task scheduled.');
        try { await load(); } catch(e) { toast('Saved, but the view could not refresh. Use Refresh to reload.'); }
      } catch(e) { error.textContent=e.message; } finally { button.disabled=false; }
    });
  });
  window.cramDashboard={
    setSession(result) { token=result.token; sessionStorage.setItem('cram.token',token); page='Overview'; query=''; filter=''; },
    hasSession:()=>Boolean(token), open,
  };
})();
